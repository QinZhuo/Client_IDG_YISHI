...
namespace IDG
{
    public class PeopleRenderer : MonoBehaviour
    {
        public List<string> clothingPath;
        private void Start()
        {
            FreshMesh();
        }
        public void FreshMesh()
        {
            ...
            foreach (var pathe in clothingPath)
            {
                var prefab = Resources.Load("Prefabs/Clothing/" + pathe);
                var obj = Instantiate(prefab) as GameObject;
                objects.Add(obj);
                skinMeshes.Add(obj.GetComponentInChildren<SkinnedMeshRenderer>());
            }
            CombineSkinned.CombineObject(gameObject, skinMeshes.ToArray());
    ...
    public static class CombineSkinned
    {
        private static int COMBINE_TEXTURE_MAX = 512;
        private static string COMBINE_DIFFUSE_TEXTURE = "_MainTex";
